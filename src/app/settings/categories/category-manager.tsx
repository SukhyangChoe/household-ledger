...
function CreateCategoryForm({
  ...
  const [state, formAction, isPending] =
    useActionState(createCategory, initialState);
  ...
  return (
    ...
    >
      <p className="font-semibold">새 카테고리 등록</p>
      ...
      <p className="mt-3 text-xs leading-5 text-gray-500">
        카테고리의 기본값은 거래 입력 시 자동
        선택되지만, 실제 거래에서는 필요에 따라
        변경할 수 있습니다.
      </p>

      <button
        type="submit"
        disabled={isPending || incomeUnavailable}
...
function CategoryRow({
  ...
  category: Category;
  rateRuleMap: Map<string, RateRule>;
  accounts: Account[];
  rateRules: RateRule[];
}) {
  const linkedRateRule = category.rate_rule_id
    ? rateRuleMap.get(category.rate_rule_id)
    : null;

  const linkedAccount = category.default_account_id
    ? accountMap.get(category.default_account_id)
    : null;
  ...
  const selectableRateRules = rateRules.filter(
    (rateRule) =>
      (rateRule.is_active &&
        rateRule.valid_to === null) ||
      rateRule.id === category.rate_rule_id,
  );
  ...
  const [
    ...
    updateAction,
    updatePending,
  ] = useActionState(updateCategory, initialState);
  ...
  const [
    ...
    deleteAction,
    deletePending,
  ] = useActionState(deleteCategory, initialState);
          ...
          {category.transaction_type === "income" ? (
            <>
              <p className="mt-2 text-xs text-gray-500">
                반영률:{" "}
                {linkedRateRule
                  ? `${linkedRateRule.name} · ${formatRate(
